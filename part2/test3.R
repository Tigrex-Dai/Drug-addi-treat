...
#DSDECOD=="PARTICIPANT COMPLETED ACTIVE PHASE OF STUDY"
statusfini <- ds %>% filter(VISITNUM<=14)
totalid <- bras$USUBJID
step0 <- right_join(statusfini, bras, by = "USUBJID")
# step01 <- subset(step0, step0$USUBJID %in% totalid)
...
# temptest <- step2 %>% filter(VISITNUM.y == 0)
# nrow(filter(temptest, temptest$USUBJID == "01_000579" & temptest$LBORRES != "NEGATIVE"))
length(totalid)
...
temptest <- preres
temptest <- select(temptest, -c(id,day1test))

# preparation
temptest$finalres <- as.factor(temptest$finalres)
task <- TaskClassif$new("respred", temptest, target = "finalres")
